...
$trainingArguments = @(
    ...
    "--default-anchor-weight-percent", "30",
...
try {
    while (-not $process.HasExited) {
        ...
        if ($generation -ge $StopGeneration) {
            $reason = if ($StopGeneration -ge 100) { "training_complete" } else { "pilot_gate_complete" }
...
$state = if ($reason -in @("time_box_complete", "training_complete", "pilot_gate_complete")) {
    "completed"
}
else {
    "failed"
