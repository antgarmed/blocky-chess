...
foreach ($artifact in @($checkpoint, $stdoutLog, $stderrLog, $statusPath)) {
    if (Test-Path -LiteralPath $artifact) {
        throw "Run artifact already exists: $artifact"
    }
}
...
$arguments = @(
    ...
    "--historical-weight-percent", "40",
    "--historical-opponents", "4",
    "--historical-opening-pairs", "1",
    "--historical-insertion-cadence", "5",
...
if (Test-Path -LiteralPath $checkpoint) {
    try {
        $nextGeneration = [int](Get-Content -LiteralPath $checkpoint -Raw |
            ConvertFrom-Json).state.next_generation
    } catch {}
}
$stderrBytes = if (Test-Path -LiteralPath $stderrLog) {
    (Get-Item -LiteralPath $stderrLog).Length
} else { -1 }
$state = if ($process.ExitCode -eq 0 -and $nextGeneration -eq 50 -and
...
$status = [ordered]@{
    ...
    checkpoint_next_generation = $nextGeneration
